<template>
  <v-app>
<!--toolbar-->
    <v-app-bar color="#001ded" class="custom-toolbar" density="prominent" scroll-behavior="hide inverted">
      ...
      >
        <v-card class="mx-auto" max-width="800px" style="background-color: rgba(0, 0, 0, 0.5);">
          <v-row no-gutters>
            ...
            <!-- 右側文字 -->
            <v-col cols="12" md="8">
              <v-card-text :style="{ whiteSpace: 'pre-line', fontSize: '1.1rem' }">
                <h2>{{ item.title }}</h2>
                <br>
...
.responsive-video {
  position: absolute; /* 使影片相對於容器絕對定位 */
  top: 0;             /* 從容器頂部開始 */
  left: 0;            /* 從容器左側開始 */
  width: 100%;        /* 讓影片寬度充滿容器 */
  height: 100%;       /* 讓影片高度自適應容器 */
